...
import { GTDProcessingResult, PluginSettings, FlowProject } from '../src/types';
...
`;
			...
			await fileWriter.addNextActionToProject(mockProject, 'New future action', true);
...
`;
			...
			await fileWriter.addNextActionToProject(mockProject, 'First action', false);
...
`;
	...
	describe('updateProjectTags', () => {
		...
		it('should preserve existing project tags', async () => {
			...
			(mockFileManager.processFrontMatter as jest.Mock).mockImplementation(
				(file, callback) => {
					const mockFrontmatter = { tags: ['project/personal', 'project/health', 'urgent'] };
					callback(mockFrontmatter);
