...
export class GTDEvaluator {
	...
	private startsWithActionVerb(action: string): boolean {
		const actionVerbs = [
			'call', 'email', 'write', 'review', 'research', 'schedule', 'book',
			'buy', 'order', 'create', 'update', 'fix', 'test', 'send', 'meet',
			'discuss', 'plan', 'analyse', 'analyze', 'prepare', 'organise', 'organize',
			'draft', 'contact', 'confirm', 'check', 'complete', 'finish', 'start',
			'reply', 'respond', 'follow', 'ask', 'request', 'submit', 'download',
			'upload', 'install', 'configure', 'set', 'read', 'watch', 'listen'
		];
	...
	private evaluateSpecificity(text: string): number {
		...
		// Has times/dates
		if (/\d{1,2}(?::\d{2})?\s*(?:am|pm)|(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)/i.test(text)) {
			score += 0.15;
		}
	...
	private summarizeResults(results: EvaluationResult[]): EvaluationSummary {
		...
		const actionQualityScore = results.reduce((sum, r) => sum + r.metrics.actionQuality, 0) / totalTests * 100;
