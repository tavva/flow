...
export class NewPersonModal extends Modal {
  ...
  constructor(plugin: FlowPlugin, description: string, onSubmit: (personName: string, description: string) => void) {
    super(plugin.app)
    this.plugin = plugin
  ...
  onOpen() {
    ...
    new Setting(contentEl)
      .setName('Person name')
      .addText(text => text.onChange(val => { this.personName = val.trim() }))
    new Setting(contentEl)
      .setName('Description')
      .addTextArea(text => text.setValue(this.description).onChange(val => { this.description = val }))
    const warningEl = contentEl.createDiv()
    warningEl.addClass('warning')
    new Setting(contentEl).addButton(btn =>
      btn.setButtonText('Create').setCta().onClick(async () => {
        if (!this.personName) {
          warningEl.setText('Please enter a person name'); warningEl.show(); return
        }
        if (await getPersonFilePath(this.plugin, this.personName)) {
          warningEl.setText('Person already exists'); warningEl.show(); return
        }
